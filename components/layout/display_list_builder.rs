...
impl FragmentDisplayListBuilding for Fragment {
    ...
    fn build_display_list_for_background_gradient(&self,
        ...
        fn get_first_tile(base: i32, start: i32, tile: i32) -> i32 {
            if start > base {
        ...
        bounds.origin.x = bounds.origin.x + bg_position_x.to_used_value(bounds.size.width);
        bounds.origin.y = bounds.origin.y + bg_position_y.to_used_value(bounds.size.height);
        ...
        let diff_x = get_first_tile(absolute_bounds.origin.x.0,
                                    bounds.origin.x.0,
                                    tile.width.0);
        let diff_y = get_first_tile(absolute_bounds.origin.y.0,
                                    bounds.origin.y.0,
                                    tile.height.0);
        let tiled_bounds = Rect::new(
            Point2D::new(bounds.origin.x - Au(diff_x), bounds.origin.y - Au(diff_y)),
            Size2D::new(absolute_bounds.size.width + Au(diff_x),
                        absolute_bounds.size.height + Au(diff_y)));
